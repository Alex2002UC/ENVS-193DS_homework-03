...
ggplot(data = Cooking, aes(x = school_day, y = minutes_spent_cooking_min, color = school_day)) +
  geom_jitter(width = 0.2, height = 0, alpha = 0.6, shape = 21) +  # Adds raw data points with jitter
...
scale_x_discrete(labels = c("N" = "Non-School Day", "Y" = "School Day")) +
  scale_color_manual(values = c("Y" = "steelblue", "N" = "darkorange")) +
...
```{r}
Tussocks_summary |>
gt() |> #Creating a Table from the data
cols_label( # Renaming columns so it's easier to read
site = "Site",
mean = "Mean",
...
tab_header(
title = "Summary statistics of average moss height by site")
